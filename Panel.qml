...
import "CalendarModel.js" as CalModel
...
//
// This clone also carries an iCalendar feed service: days with
// events grow dots under their numbers, and an upcoming list rides below
// the month grid in the same quiet register as the rest of the panel.
//
// BarWidget.qml owns the bar label and hands this panel the button to
// anchor against.
Panel {
  id: root
  moduleName: "matteodevenuto.clock"
  ipcTarget: "matteodevenuto.clock"
  manageIpc: false

  property var anchorItem: null

  // The embedded iCalendar feed service, injected by BarWidget.qml.
  ...
  readonly property var selectedEvents: calendarConfigured && service
    ? CalModel.eventsOn(service.buckets, selectedKey) : []
  readonly property date selectedDate: CalModel.parseStamp(selectedKey) || today

  // ---- Calendar feeds. The service syncs on its own timer; these read
  //      whatever it is holding at the moment.
  readonly property bool calendarConfigured: service ? service.configured === true : false
  readonly property var upcomingEvents: {
    if (!calendarConfigured || !service) return []
    return CalModel.upcoming(service.events, service.now).slice(0, 5)
  }

  function dayEvents(year, month, day) {
    if (!calendarConfigured) return []
    return CalModel.eventsOn(service.buckets, CalModel.dateKey(year, month, day))
  }

  // The dot model for one grid cell: first three live events, cancelled
  // ones left out.
  function dayDots(year, month, day) {
    var list = dayEvents(year, month, day)
    var out = []
    for (var i = 0; i < list.length && out.length < 3; i++)
      if (!list[i].cancelled) out.push(list[i])
    return out
  }

  // "14:30" for anything today, otherwise a short date leading the time —
  // all-day events carry a date or the words alone.
  function agendaWhen(event) {
    if (!event) return ""
    var time = event.allDay ? "all day" : Qt.formatDateTime(event.start, "HH:mm")
    if (CalModel.keyForDate(event.start) === String(todayKey)) return time
    var day = Qt.formatDate(event.start, "ddd d MMM")
    return event.allDay ? day : day + " · " + time
  }
  ...
  function open() {
    refresh()
    if (root.service && root.service.ensureFresh) root.service.ensureFresh()
  ...
  function addCalendar() {
    var url = root.feedDraft.replace(/^\s+|\s+$/g, "")
    if (url.indexOf("https://") !== 0 || !root.service) return
    root.service.addFeed(url, "")
    feedUrl.text = ""
  }
      ...
      Flickable {
        ...
        Column {
                ...
                Row {
                  ...
                  Repeater {
                    ...
                    Rectangle {
                      ...
                      // Up to three dots for the days that carry events;
                      // cancelled ones are skipped.
                      readonly property var dots: root.dayDots(modelData.year, modelData.month, modelData.day)
                      ...
                      color: dayMouse.containsMouse || modelData.key === root.selectedKey
                        ? Style.hoverFillFor(root.contentForeground, Color.accent)
                        : "transparent"
                      ...
                      Text {
                        anchors.centerIn: parent
                        // The number lifts a hair off center when dots move
                        // in underneath, so the pair reads as one unit.
                        anchors.verticalCenterOffset: dayCell.dots.length > 0 ? -Style.space(2) : 0
                        text: modelData.day
                      ...
                      Row {
                        visible: modelData.inMonth && dayCell.dots.length > 0
                        anchors.horizontalCenter: parent.horizontalCenter
                        ...
                        Repeater {
                          ...
                          Rectangle {
                            ...
                            radius: width / 2
                            color: {
                              var c = String(modelData.color || "")
                              if (c.length > 1 && c.charAt(0) === "#") return c
                              return Color.accent
                            }
                          }
                        }
          ...
          Item {
            ...
            Item {
              ...
              Row {
                ...
                PanelActionButton {
                  iconText: "󰃰"
                  tooltipText: root.showUpcoming ? "Show selected day" : "Show upcoming events"
                  bordered: root.showUpcoming
                  foreground: root.contentForeground
                  fontFamily: root.contentFontFamily
                  onClicked: root.showUpcoming = !root.showUpcoming
                }

                PanelActionButton {
                  iconText: "󰒓"
                  tooltipText: root.settingsOpen ? "Hide calendars" : "Manage calendars"
          ...
          Column {
            ...
            Repeater {
              model: root.service ? root.service.feeds : []
              ...
              Rectangle {
                ...
                width: parent.width
                height: Style.space(30) + (picking ? Style.space(24) : 0)
                radius: Style.cornerRadius
                ...
                Rectangle {
                  ...
                  MouseArea {
                    ...
                    onClicked: feedRow.picking = !feedRow.picking
                  }
                ...
                Row {
                  ...
                  Repeater {
                    model: root.calendarPalette

                    Rectangle {
                      required property var modelData
                      required property int index

                      readonly property bool current: String(feedRow.modelData.color || "").toLowerCase() === modelData
                      ...
                      MouseArea {
                        ...
                        enabled: !(root.service && root.service.syncing)
                        onClicked: {
                          if (root.service) root.service.setFeedColor(feedRow.modelData.url, parent.modelData)
                          feedRow.picking = false
                        }
                      }
                    }
                  }
                ...
                Row {
                  id: orderButtons
                  anchors.right: removeFeed.left
                  anchors.rightMargin: Style.space(2)
                  anchors.verticalCenter: parent.verticalCenter
                  visible: (root.service ? root.service.feeds.length : 0) > 1
                  spacing: 0
                  ...
                  PanelActionButton {
                    ...
                    iconText: "󰅃"
                    tooltipText: "Move up"
                    enabled: !(root.service && root.service.syncing) && feedRow.index > 0
                    opacity: enabled ? 1.0 : 0.3
                    foreground: root.contentForeground
                    fontFamily: root.contentFontFamily
                    onClicked: {
                      if (root.service) root.service.moveFeed(feedRow.modelData.url, feedRow.index - 1)
                    }
                  }
                  ...
                  PanelActionButton {
                    ...
                    enabled: !(root.service && root.service.syncing)
                      && feedRow.index < (root.service ? root.service.feeds.length - 1 : 0)
                    opacity: enabled ? 1.0 : 0.3
                    foreground: root.contentForeground
                    fontFamily: root.contentFontFamily
                    onClicked: {
                ...
                PanelActionButton {
                  ...
                  foreground: root.contentForeground
                  fontFamily: root.contentFontFamily
                  onClicked: { if (root.service) root.service.removeFeed(feedRow.modelData.url) }
                }
            ...
            Row {
              ...
              TextField {
                id: feedUrl
                width: parent.width - addFeed.width - Style.space(6)
                placeholderText: "Paste another calendar link…"
                enabled: !(root.service && root.service.syncing)
                foreground: root.contentForeground
                font.family: root.contentFontFamily
                onTextChanged: root.feedDraft = text
                Keys.onReturnPressed: root.addCalendar()
                Keys.onEnterPressed: root.addCalendar()
              }
              ...
              Button {
                ...
                text: "Add"
                bordered: true
                enabled: root.feedDraft.indexOf("https://") === 0
                  && !(root.service && root.service.syncing)
            ...
            Text {
              width: parent.width
              text: "Paste a link to any iCalendar (.ics) feed. Use the arrows to reorder and click a calendar's dot to change its color. The share URL can read that calendar, so keep it secret."
          ...
          Column {
            visible: root.calendarConfigured
            width: gridColumn.width
            anchors.horizontalCenter: parent.horizontalCenter
            spacing: Style.space(4)
            ...
            Text {
              visible: (root.showUpcoming ? root.upcomingEvents : root.selectedEvents).length === 0
              text: root.showUpcoming ? "Nothing coming up." : "Nothing scheduled."
              color: Qt.darker(root.contentForeground, 1.9)
              font.family: root.contentFontFamily
              font.pixelSize: Style.font.caption
            }
            ...
            Repeater {
              model: root.showUpcoming ? root.upcomingEvents : root.selectedEvents
              ...
              Rectangle {
                ...
                radius: Style.cornerRadius
                color: selectedEventMouse.containsMouse
                  ? Style.hoverFillFor(root.contentForeground, Color.accent) : "transparent"
                ...
                Rectangle {
                  ...
                  radius: width / 2
                  color: selectedEventRow.modelData.color || Color.accent
                }
                ...
                Text {
                  ...
                  anchors.verticalCenter: parent.verticalCenter
                  width: Style.space(root.showUpcoming ? 132 : 52)
                  text: root.showUpcoming
                    ? root.agendaWhen(selectedEventRow.modelData)
                    : (selectedEventRow.modelData.allDay ? "all day"
                      : Qt.formatDateTime(selectedEventRow.modelData.start, "HH:mm"))
                  color: Qt.darker(root.contentForeground, 1.5)
                  font.family: root.contentFontFamily
                  font.pixelSize: Style.font.caption
                }
                ...
                Text {
                  anchors.left: selectedTime.right
                  anchors.right: parent.right
                  anchors.rightMargin: Style.space(8)
                  anchors.verticalCenter: parent.verticalCenter
                  text: selectedEventRow.modelData.title
                    + (selectedEventRow.modelData.location ? " · " + selectedEventRow.modelData.location : "")
                  elide: Text.ElideRight
                  color: root.contentForeground
                  font.family: root.contentFontFamily
                  font.pixelSize: Style.font.bodySmall
